...
@app.route('/api/heightsAndWeights/<int:height_and_weight_id>', methods=['GET'])
def api_retrieve(height_and_weight_id) -> str:
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tableHeightsAndWeights WHERE id=%s', height_and_weight_id)
    result = cursor.fetchall()
    json_result = json.dumps(result)
...
@app.route('/api/heightsAndWeights/<int:height_and_weight_id>', methods=['PUT'])
def api_edit(height_and_weight_id) -> str:
    cursor = mysql.get_db().cursor()
    content = request.json
    input_data = (content['heightInches'], content['weightPounds'], height_and_weight_id)
...
@app.route('/api/heightsAndWeights', methods=['POST'])
def api_add() -> str:
    content = request.json

    cursor = mysql.get_db().cursor()
    input_data = (content['heightInches'], content['weightPounds'])
    sql_insert_query = """INSERT INTO tableHeightsAndWeights (heightInches, weightPounds) VALUES (%s, %s) """
    cursor.execute(sql_insert_query, input_data)
    mysql.get_db().commit()
...
@app.route('/api/heightsAndWeights/<int:height_and_weight_id>', methods=['DELETE'])
def api_delete(height_and_weight_id) -> str:
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM tableHeightsAndWeights WHERE id = %s """
    cursor.execute(sql_delete_query, height_and_weight_id)
    mysql.get_db().commit()
